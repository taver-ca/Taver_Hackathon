...
function generateCombinations(dictionary) {
  ...
  function backtrack(combination, index, lastDate, lastLocation, lastTitle, filteredDictionary) {
    ...
    // **Filter dictionary BEFORE the loop** to reduce unnecessary checks
    filteredDictionary[key] = filteredDictionary[key].filter(value => new Date(value.date) >= lastDate);

    for (const value of filteredDictionary[key]) {
      ...
      if (concertDate >= lastDate && (concertDate !== lastDate || (value.location.name === lastLocation.name && value.location.address === lastLocation.address && value.title === lastTitle))) {
        ...
        // Pass the already filtered dictionary forward
        backtrack(combination, index + 1, concertDate, value.location, value.title, filteredDictionary);
        combination.pop();
      }
    }
  }

  backtrack([], 0, new Date(0), { name: null, address: null }, null, { ...dictionary });
  return result;
}
...
export const GenerateOptimizedConcertRoute = (
  ...
  console.table(`allCombinationOfConcerts[0]: ${allCombinationOfConcerts[0]}`);
